...
pub async fn synth_inventory(app: &AppL2, global_config: &GlobalConfig) -> Result<()> {
    ...
    for (_, inventory) in app.inner.inventories.iter() {
...
pub async fn synth_playbooks(app: &AppL2, global_config: &GlobalConfig) -> Result<()> {
